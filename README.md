...
- **Render:** ReSTIR Direct + Path-Traced GI/MIS/NEE/BSDF strategy, MetalFX, an OIDN preview of the current accumulation, Pause/Resume, Restart, sample and time limits, preview scale, scattering depth, exposure, a cool/warm white-balance adjustment, filmic/Reinhard/linear-clip tone maps, and a raw/MetalFX divider. ReSTIR resamples primary direct lighting; indirect diffuse, glossy, and transmission paths continue through the path tracer. Limits of zero mean unlimited. The status bar shows completed samples, FPS, GPU milliseconds, elapsed active render time, and render dimensions.
...
- Use the official Intel Open Image Denoise 2.5.0 `RT` filter for final-frame exports. The offline path receives the converged linear HDR image and albedo/normal guides, requests high quality, and lets OIDN prefilter stochastic guide edges. It runs after sampling and before PNG display mapping or linear OpenEXR encoding.
