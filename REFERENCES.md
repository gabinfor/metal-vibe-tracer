...
**Used in:** `scripts/prepare_oidn.py`, `OIDNDenoiser`, `OIDNProgress`, `StudioController.beginOfflineDenoise`, and the Project & Export source selector. The checksum-pinned official macOS runtime is bundled without modification and loaded through OIDN's C99 ABI only for offline exports. The local adapter copies the converged `rgba32Float` accumulation, primary material albedo, and world normal to an `RT` filter as `FLOAT3`, sets `hdr=true`, `quality=OIDN_QUALITY_HIGH`, and writes the returned linear radiance into a new Metal texture. PNG display mapping occurs afterward; OpenEXR retains the linear denoised values. Raw and MetalFX export sources remain available.
...
Validation on Apple M4, September 7, 2026: `./build.sh`, the focused Studio/MaterialX/OpenUSD suite, and the complete `MTL_DEBUG_LAYER=1 python3 tests/verify.py` suite passed. Coverage includes runtime shader compilation, six scenes and four strategies, energy/furnace checks, MetalFX history, the bundled OIDN high-quality HDR filter and controller export path, project round trips, fault-injected binding rollback, area-only emitter PDFs, all imported-scene integrators, and a fresh 63,882-triangle ASWF Shader Ball import/render. The generated audit artifacts use new names and do not replace the September 4 preview.
